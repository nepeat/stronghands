...
vector<KernelRecord> KernelRecord::decomposeOutput(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int nDayWeight = (min((GetAdjustedTime() - nTime), (int64)((int64)nTimeTx > FORK_TIME ? STAKE_MAX_AGE_2 : STAKE_MAX_AGE)) - nStakeMinAge) / 86400;
...
double KernelRecord::getProbToMintStake(double difficulty, int timeOffset) const
{
    ...
    int dayWeight = (min((GetAdjustedTime() - nTime) + timeOffset, (int64)((int64)nTimeTx > FORK_TIME ? STAKE_MAX_AGE_2 : STAKE_MAX_AGE)) - nStakeMinAge) / 86400;
